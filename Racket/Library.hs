...
errorBuiltin = \es -> do
    ...
    throwError $ ThrownError $ show msg
...
checkDef expr = putResult $ case expr of
    ...
    _                  -> defError
    where defError = Left $ EvalError "first arg should be either identifier \
...
define = builtinOf2Args $ \defExpr body -> do
    ...
    let funcExpr = FuncExpr $ Func name args body
        newEnv = insertEnv name funcExpr env
    putEnv newEnv
    return Void
...
funcListBuiltin builtin = library . builtinOf2Args $ \fe le -> do
    let ms1 = if isFunc fe then []
                else [printf "first arg should be function, \
                              \%s given" (show fe)]
...
listBuiltin builtin = library $ \args -> do
    ...
    let es = getList le
    check (not $ null es) $ "function accepts non-empty lists"
    return $ builtin es
